...
export const addUser = async (data) => {
  ...
  try {
    const [confirm] = await pool.query( // pool.query 결과에서 첫번째 요소를 confirm이라는 변수에 저장
...
export const getUser = async (userId) => {
  ...
  try {
    const [user] = await pool.query(`SELECT * FROM user WHERE id = ?;`, userId); // 데이터베이스 쿼리를 실행, 배열의 첫 요소를 user변수에 할당
    /** user table에서 특정 id를 가진 사용자의 모든 정보를 가져옴 ?는 준비된 문구로 userId로 대체됨 */

    console.log(user); // debug

    if (user.length == 0) { // 쿼리가 비어있는지 확인
      return null;
    }
...
export const setPreference = async (userId, foodCategoryId) => {
  const conn = await pool.getConnection(); // 데이터베이스와의 연결을 얻는 역할
